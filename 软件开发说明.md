...
```mermaid
flowchart TB
    A["控制信号(遥控器)"] --> B[飞行状态控制器]
    B --> C[导航与轨迹层: 位置/速度目标值]
    C --> D[姿态控制层: 角速度/姿态目标值]

    D --> F[控制分配器 Control Allocator]
...
```mermaid
flowchart TB
    C["ControlAllocator::Run() 检查订阅与更新标志"] --> D["_control_allocation[selected].setControlSetpoint(c)"]
...
```mermaid
flowchart TB
    A[姿态控制器输出目标扭力与升力] --> B[Control Allocation根据自定义机型解算六“旋翼”推力]
    B --> C[计算两电机推力]

    C --> D[通过周围四“旋翼”推力计算气道阻力]
    D --> E["舵机输出pow(x,0.8)非线性映射"]
